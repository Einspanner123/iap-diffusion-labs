...
- 用什么概率路径训练
- 用什么损失拟合向量场
- 采样时如何用 guidance 控制结果

这一讲开始转向工程问题：

1. 数据太高维，怎么降维后再生成？
2. 真正的大模型里，向量场网络到底长什么样？

所以这讲回答的是“原理落地时，系统应该怎么搭”。
...
表面上看，我们从像素空间换到了潜空间，好像“换了任务”。

但本质上并没有：

- 编码器把原图映射到一个更紧凑的表示
- 扩散模型只是在这个表示空间里学习数据分布
- 解码器再把生成出的潜变量还原回图像

也就是说，LDM 的核心思想是：

> 生成任务不变，只是把“难的高维像素分布”换成“更容易建模的潜变量分布”。
...
- **扩展性好**：参数、层数、token 数都容易放大
- **多模态兼容**：图像 token、文本 token 可以自然放进统一注意力框架
- **适合大规模训练**：和现代 Transformer 训练基础设施兼容

这也是 Stable Diffusion 3、FLUX、MovieGen 等系统逐步转向 Transformer 系列架构的重要原因。
